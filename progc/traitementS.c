...
unsigned long int cligne =1;
  ...
  while (fgets(ligne, 300, data)!=NULL){//parcours tout le fichier
    ...
    av= insertion(av,lupt,h,listept,fpt,m);//on ajoute toutes les villes de fin d'étape la fonction modifie toutes les valeurs comme il faut
    }
}
